  ...
  return (
    <div className="page-shell space-y-6">
      <PageHeader
        ...
        icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-violet-400"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>}
        actions={
          <div className="flex items-center gap-2 rounded-full border border-border-soft bg-surface-1 p-1 shadow-sm">
            {FILTERS.map(f=><button key={f.label} onClick={()=>setFilter(f.value)} className={`rounded-full px-4 py-2 text-[12.5px] font-semibold transition-all ${filter===f.value ? 'bg-gradient-to-r from-violet-500 to-pink-500 text-white shadow-sm border border-border-soft' : 'text-ivory-faint hover:text-ivory hover:bg-surface-2'}`}>{f.label}</button>)}
          </div>
        }
      />
        ...
        {loading ? <div className="p-12 text-center text-[13px] text-ivory-faint">Loading KYC queue…</div> : items.length===0 ? <div className="p-16 text-center"><div className="grid place-items-center h-14 w-14 mx-auto rounded-2xl bg-surface-1 border border-border-soft">📭</div><div className="mt-3 font-semibold">No applications</div><div className="text-[13px] text-ivory-faint mt-1">No astrologers in this filter.</div></div> : (
          <>
                ...
                <tbody className="divide-y divide-border-soft/60">
                  {items.map(it=>(
                    <tr key={it.astrologer_id} className="hover:bg-surface-1/50 transition-colors">
                      ...
                      <td className="px-5 py-4 text-right"><button onClick={()=>setActive(it)} className="inline-flex h-8 items-center gap-1.5 rounded-full border border-border-soft bg-gradient-to-r from-violet-500 to-pink-500 text-white px-4 text-[12px] font-bold hover:bg-white transition-colors">Review →</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="grid gap-3 p-3 md:hidden">
              {items.map(it=>(
                <div key={it.astrologer_id} className="rounded-[20px] border border-white/10 bg-white/5 backdrop-blur-xl p-4">
                  ...
                  <button onClick={()=>setActive(it)} className="mt-3 w-full h-9 rounded-full bg-gradient-to-r from-violet-500 to-pink-500 text-white text-[12px] font-bold">Review application</button>
                </div>
              ))}
            </div>
          </>
...
function ReviewModal({item,onClose,onDecide}:{item:KYCReviewItem; onClose:()=>void; onDecide:(id:string, approve:boolean, notes:string)=>void}){
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/70 backdrop-blur-xl p-4 animate-fade-in" onClick={onClose}>
      <div className="w-full max-w-[560px] animate-pop-in rounded-[26px] border border-white/12 bg-white/6 backdrop-blur-2xl p-6 sm:p-7 shadow-[0_24px_80px_rgba(0,0,0,0.5)]" onClick={e=>e.stopPropagation()}>
        ...
        <div className="mt-6 grid gap-3 rounded-[20px] border border-white/10 bg-white/5 backdrop-blur-xl p-4 text-[13px] sm:grid-cols-[110px_1fr] [&>.k]:text-[11px] [&>.k]:font-bold [&>.k]:uppercase [&>.k]:text-ivory-faint">
        ...
        {pending ? (
          <>
            <label className="mt-5 block text-[12px] font-semibold text-ivory-dim">Reviewer notes (optional)<textarea rows={3} value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Add context for audit trail…" className="mt-2 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-3.5 text-[13px] outline-none focus:border-saffron focus:bg-surface-2" /></label>
