...
export default function CouponsPage(){
  ...
  return (
    <div className="page-shell space-y-6">
      <PageHeader title="Coupons" subtitle="Discount codes for wallet recharges and consultations. Create, manage expiry and usage limits." icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-violet-400"><path d="M2 9V7a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v2a2 2 0 0 0 0 4v2a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-2a2 2 0 0 0 0-4z"/><path d="M9 9h.01"/><path d="M15 9h.01"/></svg>} />
      ...
      <Card className="p-5 sm:p-6">
        ...
        <form onSubmit={create} className="mt-4 grid gap-3 sm:grid-cols-5 items-end">
          <label className="text-[11px] font-semibold text-ivory-dim">Code<input value={code} onChange={e=>setCode(e.target.value.toUpperCase())} placeholder="WELCOME50" required className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px] font-mono font-bold outline-none focus:border-saffron" /></label>
          <label className="text-[11px] font-semibold text-ivory-dim">Type<select value={type} onChange={e=>setType(e.target.value)} className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px] outline-none focus:border-saffron"><option value="percent">Percent %</option><option value="flat">Flat ₹</option></select></label>
          <label className="text-[11px] font-semibold text-ivory-dim">Value<input value={value} onChange={e=>setValue(e.target.value)} placeholder={type==='percent'?'50':'100'} required className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px] outline-none focus:border-saffron" /></label>
          <label className="text-[11px] font-semibold text-ivory-dim">Min amount<input value={minAmount} onChange={e=>setMinAmount(e.target.value)} className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px] outline-none focus:border-saffron" /></label>
          <button type="submit" className="h-11 rounded-xl bg-gradient-to-r from-violet-500 to-pink-500 text-white px-5 text-[13px] font-bold hover:bg-white shadow-sm">Add coupon</button>
        </form>
      </Card>
      ...
      {loading ? <Card><div className="p-16 text-center text-[13px] text-ivory-faint">Loading coupons…</div></Card> : (
        <div className="overflow-hidden rounded-[24px] border border-white/10 bg-white/4 backdrop-blur-2xl shadow-sm">
          ...
          <div className="grid gap-3 p-3 md:hidden">
            {rows.map(c=>(
              <div key={c.id} className="rounded-[20px] border border-white/10 bg-white/5 backdrop-blur-xl p-4">
    ...
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/60 backdrop-blur-xl p-4" onClick={onClose}>
      <div className="w-full max-w-[480px] animate-pop-in rounded-[26px] border border-white/12 bg-white/6 backdrop-blur-2xl p-6 shadow-2xl" onClick={e=>e.stopPropagation()}>
        ...
        <div className="mt-5 grid gap-4">
          <label className="text-[12px] font-semibold text-ivory-dim">Description<input value={description} onChange={e=>setDescription(e.target.value)} className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px] outline-none focus:border-saffron" /></label>
          <div className="grid sm:grid-cols-2 gap-3"><label className="text-[12px] font-semibold">Value<input value={value} onChange={e=>setValue(e.target.value)} className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px]" /></label><label className="text-[12px]">Max discount<input value={maxDiscount} onChange={e=>setMaxDiscount(e.target.value)} placeholder="No cap" className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px]" /></label></div>
          <div className="grid sm:grid-cols-2 gap-3"><label className="text-[12px]">Min amount<input value={minAmount} onChange={e=>setMinAmount(e.target.value)} className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px]" /></label><label className="text-[12px]">Usage limit (0=∞)<input value={usageLimit} onChange={e=>setUsageLimit(e.target.value)} className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px]" /></label></div>
          <label className="text-[12px] font-semibold">Expires on<input type="date" value={expiresOn??''} onChange={e=>setExpiresOn(e.target.value)} className="mt-1.5 h-11 w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-3.5 text-[13px]" /></label>
        </div>
