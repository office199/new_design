  ...
  return (
    <div className="page-shell space-y-6">
      <PageHeader
        ...
        icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-emerald-400"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>}
                    ...
                    <tr key={c.id} className="hover:bg-surface-1/50">
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="inline-flex items-center gap-2">
                          ...
                          <button disabled={savingId===c.id} onClick={()=>save(c)} className="h-8 rounded-full bg-gradient-to-r from-violet-500 to-pink-500 text-white px-4 text-[12px] font-bold hover:bg-white disabled:opacity-50">{savingId===c.id?'Saving…':'Save'}</button>
                        </div>
                      </td>
                    </tr>
                  )
            ...
            {filtered.map(c=>{
              ...
              return (
                <div key={c.id} className="rounded-[20px] border border-white/10 bg-white/5 backdrop-blur-xl p-4">
                  ...
                  <button disabled={savingId===c.id} onClick={()=>save(c)} className="mt-3 w-full h-10 rounded-xl bg-gradient-to-r from-violet-500 to-pink-500 text-white text-[12px] font-bold">{savingId===c.id?'Saving…':'Save pricing'}</button>
                </div>
              )
            })}
          </div>
